...
BRANCH="ai-improvements-$(date +%Y%m%d)"
...
# ─── Commit ───────────────────────────────────────────────────────────────────
COMMIT_MSG="test: AI-generated improvements — $(date +%Y-%m-%d)

Auto-generated by Claude. See PR description for details."
...
gh pr create \
  ...
  --head "$BRANCH" \
  --label "ai-generated" \
  --label "test-improvement" 2>/dev/null || \
gh pr create \
  --title "AI Test Improvements — $(date +%Y-%m-%d)" \
  --body "$PR_BODY" \
  --base main \
